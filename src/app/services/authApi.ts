...
export const authApi = {
  register(input: { fullName: string; phoneNumber: string; barangayCode: string }) {
    return request<{ message: string; phoneNumber: string; expiresInSeconds: number }>("/auth/register", {
      method: "POST",
      body: JSON.stringify(input),
    });
  },
  resendOtp(input: { phoneNumber: string }) {
    return request<{ message: string; phoneNumber: string; expiresInSeconds: number }>("/auth/resend-otp", {
      method: "POST",
      body: JSON.stringify(input),
    });
  },
