...
function HeroMapStage() {
  ...
  useEffect(() => {
    mapFrameRef.current?.setAttribute('inert', '');
  }, []);
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      if (ticking || !isStageInView) return;
      ticking = true;
  ...
  const mapPlaceholder = <div className="landing-hero-map-placeholder" />;
  ...
  const topMap = inView ? (
    <React.Suspense fallback={mapPlaceholder}>
      <LazyIncidentMap
        ...
        zoom={18}
        height="100%"
        viewportKey="landing-hero-map-top"
        forceLight
      />
    </React.Suspense>
  ) : (
  ...
  const bottomMap = inView ? (
    <React.Suspense fallback={mapPlaceholder}>
      <LazyIncidentMap
        ...
        zoom={18}
        height="100%"
        viewportKey="landing-hero-map-bottom"
        forceLight
      />
    </React.Suspense>
  ) : (
