    ...
    <div className="p-5 bg-[var(--surface-container-lowest)] min-h-full">
      <div className="flex flex-col items-start justify-between mb-3.5 gap-2.5 md:flex-row md:items-center">
        ...
        <button
          ...
          className="flex w-full justify-center items-center gap-1.5 bg-[var(--surface-container-lowest)] border border-[var(--outline-variant)]/30 rounded-lg px-3.5 py-2 cursor-pointer text-[var(--on-surface-variant)] text-xs font-semibold md:w-auto"
        >
      ...
      <div className="grid grid-cols-2 gap-2.5 mb-3">
        <div className="bg-[var(--surface-container-lowest)] border border-[var(--outline-variant)]/30 rounded-[10px] px-3 py-2.5">
          <div className="text-[var(--outline-variant)] text-[10px] font-bold tracking-[0.06em] uppercase">{t('superadmin.auditLogs.totalEntries')}</div>
          <div className="text-[var(--on-surface)] text-2xl font-bold mt-0.5">{logs.length}</div>
        </div>
        <div className="bg-[var(--surface-container-lowest)] border border-[var(--outline-variant)]/30 rounded-[10px] px-3 py-2.5">
          <div className="text-[var(--outline-variant)] text-[10px] font-bold tracking-[0.06em] uppercase">{t('superadmin.auditLogs.actionsSeen')}</div>
          <div className="text-[var(--on-surface)] text-2xl font-bold mt-0.5">{Object.keys(actionCounts).length}</div>
        </div>
      </div>

      <div className="flex flex-col items-stretch gap-2 mb-3 bg-[var(--surface-container-lowest)] border border-[var(--outline-variant)]/30 rounded-[10px] px-3 py-2.5 md:flex-row md:items-center md:flex-wrap">
        <Filter size={13} color="#6B7280" />
        <select
          title="Filter by action"
          value={actionFilter}
          onChange={(event) => setActionFilter(event.target.value)}
          className="w-full md:w-auto px-2.5 py-[7px] border border-[var(--outline-variant)]/40 rounded-lg text-xs text-[var(--on-surface)]"
        >
          {ACTIONS.map((action) => (
        ...
        <button
          ...
          className="w-full md:w-auto border border-[var(--outline-variant)]/30 rounded-lg px-2.5 py-[7px] bg-[var(--surface-container-lowest)] text-[var(--on-surface-variant)] text-xs font-semibold cursor-pointer"
        >
        ...
        <button
          ...
          className={`w-full md:w-auto md:ml-auto border border-[var(--outline-variant)]/30 rounded-lg px-2.5 py-[7px] bg-[var(--surface-container-lowest)] text-[var(--on-surface-variant)] text-xs font-semibold ${exportButtonsDisabled ? 'cursor-not-allowed opacity-60' : 'cursor-pointer opacity-100'}`}
        >
        ...
        <button
          ...
          disabled={exportButtonsDisabled}
          className={`w-full md:w-auto border border-[var(--outline-variant)]/30 rounded-lg px-2.5 py-[7px] bg-[var(--surface-container-lowest)] text-[var(--on-surface-variant)] text-xs font-semibold ${exportButtonsDisabled ? 'cursor-not-allowed opacity-60' : 'cursor-pointer opacity-100'}`}
        >
          {exportingCsv ? t('superadmin.auditLogs.exportingCsv') : t('superadmin.auditLogs.exportCsv')}
        </button>
      </div>

      <div className="bg-[var(--surface-container-lowest)] border border-[var(--outline-variant)]/30 rounded-xl overflow-hidden">
            ...
            <tbody>
              {logs.length === 0 ? (
                ...
                <tr
                  ...
                  className={`cursor-pointer hover:bg-[var(--surface-container-low)] ${index < logs.length - 1 ? 'border-b border-[var(--outline-variant)]/15' : ''}`}
                >
                  ...
                  <td className="px-3 py-2.5 text-[var(--outline)] font-mono text-[10px] max-w-[420px]">
        ...
        <div className="flex flex-col items-start gap-2 px-3 py-2.5 border-t border-[var(--outline-variant)]/20 bg-[var(--surface-container-low)] text-xs md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={page === 1}
              className={`border border-[var(--outline-variant)]/30 rounded-[7px] px-2.5 py-1.5 bg-[var(--surface-container-lowest)] text-[var(--on-surface-variant)] text-xs font-semibold ${page === 1 ? 'cursor-not-allowed opacity-60' : 'cursor-pointer opacity-100'}`}
            >
              {t('superadmin.auditLogs.prev')}
            </button>
            <span className="text-[var(--outline)] self-center">{t('superadmin.auditLogs.page', { page, total: totalPages })}</span>
            <button
              onClick={() => setPage((current) => Math.min(totalPages, current + 1))}
              disabled={page >= totalPages}
              className={`border border-[var(--outline-variant)]/30 rounded-[7px] px-2.5 py-1.5 bg-[var(--surface-container-lowest)] text-[var(--on-surface-variant)] text-xs font-semibold ${page >= totalPages ? 'cursor-not-allowed opacity-60' : 'cursor-pointer opacity-100'}`}
            >
              {t('common.next')}
            </button>
          </div>
      ...
      {selectedLog ? (
        ...
        >
          <div
            ...
            className="w-[min(560px,100vw)] h-full bg-[var(--surface-container-lowest)] shadow-[-8px_0_24px_rgba(15,23,42,0.2)] flex flex-col"
          >
            <div className="px-4 py-3.5 border-b border-[var(--outline-variant)]/30 flex items-center justify-between">
              <div>
            ...
            <div className="p-4 overflow-y-auto grid gap-3">
              <div className="bg-[var(--surface-container-low)] border border-[var(--outline-variant)]/30 rounded-[10px] px-3 py-2.5">
                <div className="text-[var(--outline-variant)] text-[10px] font-bold tracking-[0.06em] uppercase">{t('superadmin.auditLogs.action')}</div>
                <div className="text-[var(--on-surface)] text-sm font-bold">{selectedLog.action}</div>
              </div>
              <div className="bg-[var(--surface-container-low)] border border-[var(--outline-variant)]/30 rounded-[10px] px-3 py-2.5">
                <div className="text-[var(--outline-variant)] text-[10px] font-bold tracking-[0.06em] uppercase">{t('superadmin.auditLogs.target')}</div>
              ...
              <div className="bg-[var(--surface-container-low)] border border-[var(--outline-variant)]/30 rounded-[10px] px-3 py-2.5">
                <div className="text-[var(--outline-variant)] text-[10px] font-bold tracking-[0.06em] uppercase">{t('superadmin.auditLogs.actor')}</div>
